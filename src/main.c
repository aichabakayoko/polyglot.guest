...
void drawProgress() {
    ...
    // Press [M] shortcut handler to return to Main Menu
    if (IsKeyPressed(KEY_M)) {
        state.currentScreen = MENU_SCREEN;
        return;
    }

    // 1. Screen Title (Centered at the top)
    ...
    // 6. Star Ratings (1-5 stars) based on total score milestones
    int starsEarned = 1;
    ...
    // 4. Overview Panel (Center Column)
    ...
    // Mock breakdowns for Flashcard / Scenario Scores based on performance
    ...
    // Correct & Wrong Answers (Color-Coded)
    ...
    // 5. Mastery by Category Panel (Right Column)
    ...
    // Progress bar calculations using accuracy layout math
    ...
    DrawRectangle(barX, masteryBox.y + 110, barWidth, barHeight, DARKGRAY); // Background
    int fillWidthBasics = (int)((accuracyPercent / 100.0f) * barWidth);
    DrawRectangle(barX, masteryBox.y + 110, fillWidthBasics, barHeight, GREEN); // Fill

    // Row 2: Marketplace (Staged/Mocked for upcoming expansions)
    ...
    // Row 3: Travel (Staged/Mocked for upcoming expansions)
    ...
    // 7. Interactive Controls Buttons (Bottom Row)
